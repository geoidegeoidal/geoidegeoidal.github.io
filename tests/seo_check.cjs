...
for (const file of ['index.html', 'code.html', 'maps.html', 'blog.html', 'python/gis/2025/01/02/mi-primer-analisis.html', 'conmapas/gis/2026/09/22/la-feria-queda-a-quince-minutos.html']) {
  ...
  assert.equal(graph[2]['@type'], file === 'index.html' ? 'ProfilePage' : /\/20\d{2}\//.test(file) ? 'BlogPosting' : 'CollectionPage');
}
