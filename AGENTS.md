...
- Dirección artística aprobada 2026-09-22: atlas impreso en movimiento. Globo turquesa/violeta, aro cromático CSS lento (100 s), superficies arena/lila/salvia y retrato con sombra de tinta violeta. Conservar DM Sans, textos y cartografías originales; Cursos sigue amarillo plano sin ondas. El aro solo corre con data-motion=on e is-in-view, se pausa con tab-hidden y queda estático sin JS/reduced-motion. Reutilizar el IntersectionObserver del atlas; no crear otro loop. Proyectos y áreas tecnológicas usan reveal existente, con foco inmediato sin transición. Ver tests/art_direction_check.cjs y tests/UX_ART_REVIEW.md.

- Auditoría Laws of UX 2026-09-19 y correcciones 2026-09-22: objetivos táctiles ≥44 px en móvil (bloque al final de style.css + motion.css), nombres accesibles únicos en CTAs repetidos (aria-label con tool.name/project.name) y nav unificado a «Perfil» (ruta `/` y ancla `#bio` conservadas). Pendiente editorial: densidad de Herramientas en móvil (matriz de 20 herramientas es diseño aprobado, no tocar sin petición). Portada tiene canvas de globo y arcade; las pruebas del globo deben seleccionar `.atlas-canvas`. Servir imágenes de la galería social en WebP (originales JPG conservados, excluidos los no referenciados: imagen_jorge.png e infidelidad-santiago.jpg).
- Pruebas locales en Windows: Ruby 3.3 y Jekyll 3.10 funcionan. `jekyll serve --detach` no funciona porque Ruby Windows no implementa `fork`; para automatización construir `_site` y servirlo temporalmente con `python -m http.server`. `monograph_check.cjs` omite la edición monográfica mientras permanezca archivada.
